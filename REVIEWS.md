...
## 2026-09-08 · Dark-first redesign · build, then three reviews · Orchestrator

The design handoff in `.design_handoff/` replaces the print-inspired visual system wholesale. Structure, routes, IA and copy unchanged; the floor's isometric plan unchanged. `DESIGN.md` is rewritten as the spec of record and the old one preserved at `docs/design-legacy.md`.

**Gates changed.** `qa:contrast` rewritten to measure real painted pixels (nothing on this site sits on one flat colour any more); `qa:worlds` → `qa:schemes` (the world axis is gone, the scheme axis carries more weight because the site is now dark-first); `qa:plate` retired with the plate it measured; `qa:glyphs` repointed from a display/body axis to proportional/mono.

**Defects the gates found in the build, all fixed:** seven lamp role marks sitting under seven nameplates at 1.45:1; agent hit targets under 44×44 at every width the two-up row existed at; one cabin's name inside the other cabin's button; the card slot reflowing 64px at 360; fragment links landing underneath the sticky header.

### Design Lead reviewer → CHANGES REQUESTED, 4 must-fix. All 4 closed.

1. **Every glass surface lost its blur in Chrome, Edge and Firefox** — the built CSS carried seven `-webkit-backdrop-filter` declarations and zero standard ones, and the `@supports not` fallback stayed inert because those engines *do* support the property. Source was correct throughout; `astro check` and every gate were clean. Cause: Tailwind v4's Lightning CSS pass collapses a prefixed/unprefixed pair to whichever is declared last, and cannot autoprefix a `var()` value at all. Fixed by making the pair unmergeable — the standard declaration in the rule, the `-webkit-` one in its own `@supports` block — and `qa:schemes` now asserts the computed value on the rendered page.
2. **The mid cone volume hid the empty chair**, which is the default station and the argument of the page. It was the one element added rather than transcribed. Removed; the handoff's two volumes restored, with the chair painted between them.
3. **The cabin names had left the wall.** The earlier −19 → −32 fix cleared Tanya's button by moving the name *off* the plane. Moved along the wall instead (−46, −23), which is the axis that keeps the treatment intact; `check-floor-pointer` still clean.
4. **The home page's closing address was the one address on the site not set in mono.** Fixed, along with three of seven role marks that had been redrawn from prose and collapsed seven forms into four; the handoff's own shapes are now transcribed.

Also taken: ghost pills on `--accent-wash` rather than the neutral glass fill; the offer block left-aligned; the CTA reading title-first; the case studies filling the content width; the thin page's solo screenshot sized to the copy beside it; the stage track keyed to a container query rather than the viewport.

### Perf & A11y auditor → 3 must-fix, 4 should-fix. All closed.

M1/M2 are the `backdrop-filter` bug and its consequence: nav labels at 3.09–3.82:1 over content scrolling under a translucent header. `--head` raised .72 → .9 (item 77). M3: LCP 2.36s on `/sahib/` against a 2.0s budget, 100% render delay — the reveal was hiding content already in the first viewport. Elements intersecting at observer setup are no longer hidden at all; LCP now 1.52s. S1: the two-hue focus ring collapsed to one hue on the ten floor stations and every accent pill, because component box-shadows outranked the ring — both now compose. S2: the live region announced one unbroken string (`textContent` over block children); `innerText` now. S3: the no-backdrop-filter fallback resolved to flat `--ink` on engines without `color-mix` — 498 failures at a tightest of 1.53:1 — now literal per-scheme values. S4: `/` had no `aria-current` anywhere; the wordmark carries it. Clean and confirmed by the auditor: keyboard order, screen-reader semantics, reduced motion (three attempts to leave content hidden, none succeeded), no-JS, touch targets.

### Gate auditor → verdict: the suite was WEAKER. 8 of 13 findings closed; the verdict is accepted.

The auditor found no gate bent to fit the implementation, and confirmed the two deletions were honest. It found `qa:contrast` **passing while measuring nothing**, reproduced three ways: an unmeasurable run was neither counted, printed nor failed. Closed — the gate now verifies its own blanking rule won, gives clipped scroller content a second pass with the container moved (the coverage map's product names were invisible to it at 360 and 390, and those are the marks the retired `qa:plate` had been extended to protect), measures the skip link focused and the sticky header over content at three scroll depths, holds a per-route floor on the run count, and fails by name on anything it cannot measure. Widths 2 → 5. Runs 1,858 → **5,858**. `qa:schemes` now pins every token in both schemes rather than six, closing the auditor's finding that the old gate's published-figure assertion had been lost. `qa:glyphs` fails if either face's rules disappear from the CSS scan. CI's hand-copied gate list — the reason two gates had never run on a pull request — is one `npm run qa` step.

The auditor also proved that `--accent`'s deviation from the handoff was argued from a worst-case calculation rather than a measured failure. Reverted to the published `#8B2E96`; it fails nothing across 5,858 runs. Two deviations remain, both measured, both raised as items 75 and 77.
